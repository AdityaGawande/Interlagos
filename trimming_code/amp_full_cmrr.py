    ...
    while k < 5:
        ...
        if k == 0:
           ...
           # turn_off_smu_output()
            ...
            gainy_acc = 0
            average_window=40
            print("iteration, gainx_avg, gainy_avg, gainx, gainy")
            for l in range(average_window):
               r1=[]
               r2=[]
               ...
               for i in range(2):
                    ...
                    smu.write(f"smua.source.levelv = {vref_1}")
               ...
               gainx, gainy = gain_x_gain_y.compute_gain_xy(r3, r2, r1)
               if (l < 5):
                   gainx_acc = gainx_acc
                   gainy_acc = gainy_acc
                   print(l, ",", f"\t{gainx_acc/(l+1):.4f}", ",", f"\t{gainy_acc/(l+1):.4f}", ",", f"\t{gainx:.4f}", ",", f"\t{gainy:.4f}")
               else:
                   gainx_acc = gainx_acc + gainx
                   gainy_acc = gainy_acc + gainy
                   print(l, ",", f"\t{gainx_acc/(l-4):.4f}", ",", f"\t{gainy_acc/(l-4):.4f}", ",", f"\t{gainx:.4f}", ",", f"\t{gainy:.4f}")
            #    print(l, ",", gainx_acc/(l+1), ",", gainy_acc/(l+1), ",", gainx, ",", gainy)
            gainx_avg = gainx_acc/(average_window-5)
            gainy_avg = gainy_acc/(average_window-5)
            row = int(arr1[j])
            print(row,j, k)
            cell_value_pairs = [
                (f"C{row}", gainx_avg),
